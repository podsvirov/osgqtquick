...
MeasureToolHandlerEx::MeasureToolHandlerEx(osg::Group* group, osgEarth::MapNode* mapNode)
    : _mouseDown(false)
    , _group(group)
...
void MeasureToolHandlerEx::setMapNode(MapNode* mapNode)
{
...
void MeasureToolHandlerEx::rebuild()
{
...
bool MeasureToolHandlerEx::getIsPath() const
{
    return _isPath;
}

void MeasureToolHandlerEx::setIsPath(bool path)
{
    if (_isPath != path) {
        _isPath = path;
        _finished = true;
...
GeoInterpolation
MeasureToolHandlerEx::getGeoInterpolation() const
{
    return _geoInterpolation;
}

void MeasureToolHandlerEx::setGeoInterpolation(GeoInterpolation geoInterpolation)
{
...
void MeasureToolHandlerEx::setLineStyle(const Style& style)
{
    _feature->style() = style;
    _featureNode->init();
}

bool MeasureToolHandlerEx::handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
{
    if (ea.getHandled()) {
        return false;
    }
    ...
    if (ea.getEventType() == osgGA::GUIEventAdapter::PUSH && ea.getButton() == _mouseButton) {
        ...
        if (osg::equivalent(ea.getX(), _mouseDownX, eps) && osg::equivalent(ea.getY(), _mouseDownY, eps)) {
            double lon, lat, alt;
            if (getLocationAt(view, ea.getX(), ea.getY(), lon, lat, alt)) {
                if (!_gotFirstLocation) {
                    _finished = false;
                    clear();
                    _gotFirstLocation = true;
                    _firstPos = osg::Vec3(lon, lat, alt);
                    ...
                    _secondPos = osg::Vec3(lon, lat, alt);
...
bool MeasureToolHandlerEx::getLocationAt(osgViewer::View* view, double x, double y, double& lon, double& lat, double& alt)
{
...
void MeasureToolHandlerEx::clear()
{
    //Clear the locations
...
void MeasureToolHandlerEx::setMouseButton(int mouseButton)
{
    _mouseButton = mouseButton;
}

int MeasureToolHandlerEx::getMouseButton() const
{
    return _mouseButton;
}

void MeasureToolHandlerEx::addEventHandler(MeasureToolEventHandler* handler)
{
    _eventHandlers.push_back(handler);
}

void MeasureToolHandlerEx::fireDistanceChanged()
{
    double distance = 0;
